...
public class PasswordStrengthStatus extends JPanel {
    ...
    private void checkPassword(String password) {
        ...
        if (type == 0) {
            label.setText("ninguna");
            label.setVisible(false);
        } else {
            label.setVisible(true);
            if (type == 1) {
                label.setText("Muy debil");
            } else if (type == 2) {
                label.setText("Medio");
            } else {
...
/*
    ...
    */  // el metodo alterior debe retornar un int para que se pueda usar en Register.java
    public int initPasswordField(JPasswordField txt) {
        if (documentListener == null) {
            documentListener = new DocumentListener() {
                @Override
                public void insertUpdate(DocumentEvent e) {
